...
class Teenager: Babysitter {
    ...
    func playCandyland(_ numberOfTimes: Int) {
        if responsible || patience == .high {
            print("Teenager will play candy land \(numberOfTimes) times.")
        }
        else if patience == .medium {
            print("Teenager will play candy land 2 times.")
        }
        else {
            print("Teenager will play candy land 1 time.")
        }
    }
...
var mike = Teenager(age: 15, responsible: false, patience: .medium)
mike.playCandyland(5)
let willRead = mike.read("Dat", firstLine: "ninja is whack", asleep: false)
...
class Animal: Adorable {
    ...
    var size: Size { get { return .tiny } }
    var softFur: Bool { get { return true } }
    
    func frolick() {
        print("\(species) like to walk around.")
    }
    
    func curlIntoSmallBall() {
        print("\(species) crawls into a ball.")
    }
}
var pic = UIImage(named: "frolick.jpg")
let mouse = Animal(species: "Mouse", numberOfLegs: 4)
mouse.frolick()
mouse.size
mouse.curlIntoSmallBall()
//: __Problem 3__
//:
...
class ScrubJay: Hoarder {
    ...
    func cache(_ foodItem: String) -> String {
        return "ScrubJay is hoarding \(foodItem)s."
    }
    
    func pilfer() -> String {
        return "ScrubJay stole seeds."
    }
}
...
let minion = Minion(name: "Michael")
minion.cheat()
...
extension UIColor
{
    ...
    func pistachio() -> UIColor {
        return UIColor(red: 147, green: 197, blue: 114, alpha: 255)
    }
}

let pistachio = UIColor.black
pistachio.pistachio()
