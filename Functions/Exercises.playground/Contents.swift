...
var word = "bologna"
var lastCharacter = word.remove(at: word.index(before: word.endIndex))
//: __Problem 2__
//:
...
func combineLastCharacter(words: [String]) -> String {
    var newString = ""
    for word in words {
        newString.append(word.last!)
    }
    return newString
}

let combined = combineLastCharacter(words: nonsenseArray)
//: __Problem 3__
//:
...
func decimalDigitCharacterSet(digit: String) -> Bool {
    return true
}
...
func removeDirtyWords(dirtyWords: [String]) -> [String] {
    ...
    for dirtyWord in dirtyWords {
        if dirtyWord.count != 4 {
            newStrings.append(dirtyWord)
        }
    }
    return newStrings
}

let newDirtyWordsArray = removeDirtyWords(dirtyWords: dirtyWordsArray)
//: __Problem 5__
//:
...
class MovieArchive {
    func filterByDirector(director: String, moviess: Dictionary<String, String>) -> [String] {
        var newMovies = [String]()
        for (movie, guy) in moviess {
            if guy == director {
...
let moviesBy = archive.filterByDirector(director: "Christopher Nolan", moviess: movies)
