...
let reverseSurnames = surnames.sorted(by: { $0 > $1 })
reverseSurnames
...
let reverseAverages = battingAverages.sorted(by: { $0 > $1})
reverseSurnames
...
let divisibleBy12 = numbersAsStrings.filter({(number: String) -> Bool in
    Int(number)! % 12 == 0
})
divisibleBy12
...
let divisibleBy12Consice = numbersAsStrings.filter({
    Int($0)! % 12 == 0
})
divisibleBy12Consice
//: __Problem 4__
//:
...
let micronsLessThan20 = particleSizesInMicrons.filter({(particle: Int) -> Bool in
    particle < 20
})
micronsLessThan20
...
let prices = pricesInRupees.map({(price:Int) -> String in
    return "$\(price / 66)"
})
prices
//: __Problem 6__
...
let newTimes = oldTimes.map({(time: String) -> String in
    let newTimeInt = timeIntervalFromString(time) + 13
    return timeStringFromInterval(newTimeInt) as String
})
